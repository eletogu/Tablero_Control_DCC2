...
# Estilos CSS personalizados para la Dirección de Cobro Coactivo 2
st.markdown("""
    <style>
...
if not links:
    st.markdown("""
        <div class="card-vacio">
            ...
            <p>Los enlaces de OneDrive no están configurados en los secretos de la aplicación.</p>
            <p>Por favor, configure <b>[links_onedrive]</b> en los 'Secrets' de Streamlit Cloud.</p>
        </div>
    """, unsafe_allow_html=True)
else:
    ...
    if "FUIC" in bases_datos:
        ...
        # Identificación dinámica de columnas críticas
        ...
        # Preparación de datos para el cruce
        ...
        # Análisis proceso por proceso
        for _, row in df_f.iterrows():
            p_id = row.get('ID_B')
            if not p_id: continue
            
            etapa = str(row.get(c_etapa, "")).upper()
            responsable = row.get(c_sust, "POR ASIGNAR")
            
            # --- Fecha Avoco (Hito para Mandamiento) ---
            f_avoco = pd.NA
            if not df_p.empty:
            ...
            # 2. Fuerza Ejecutoria (5 años desde ejecutoria sin Not MP)
            f_ejec = row.get(c_f_ejec)
            ...
            # 3. Medidas Cautelares (10 años para Inmuebles)
            status_medida = "OK"
            if not df_b.empty:
        ...
        with tab_alertas:
            st.subheader("Expedientes con Alertas Críticas")
            # Filtro para mostrar solo lo que NO está OK
            ...
            if not df_crit.empty:
                ...
                st.success("✅ Todos los procesos están dentro de los términos legales en los filtros actuales.")

        with tab_gestores:
            st.subheader("Ranking de Alertas por Funcionario")
        ...
        st.error("Error Crítico: No se pudo cargar el archivo FUIC. Verifique los enlaces de OneDrive.")
